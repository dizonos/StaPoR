...
class MainForm(QMainWindow):
    ...
    def perenesti_db(self):
        name, ok_pressed = QInputDialog.getText(self, "Создать класс",
                                                "Введите название класса")
        if ok_pressed:
            with open(f'db/{name}.db', 'w') as file:
                pass
            file.close()
        else:
            return
        self.data = sqlite3.connect(f'db/{name}.db')
        self.data.cursor().execute("""CREATE TABLE pupils (
